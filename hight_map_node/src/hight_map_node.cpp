...
HightMapNode::HightMapNode() : Node("HightMapNode") {
  ...
  rivz_puber_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/local_hight_map", 10);
  local_hight_map_ = pcl::PointCloud<pcl::PointXYZ>::Ptr(new pcl::PointCloud<pcl::PointXYZ>());
}
...
void HightMapNode::hight_map_callback(const unitree_go::msg::HeightMap::ConstSharedPtr msg) {
  ...
  int index;
  local_hight_map_->width = msg->width;
  local_hight_map_->height = msg->height;
...
void HightMapNode::convert_cloudpoints() {
    tf2::Transform transform_local_to_world = transform_world_.inverse() * transform_local_;
    Eigen::Matrix4f eigen_transform;
    pcl_ros::transformAsMatrix(transform_local_to_world, eigen_transform);

    pcl::PointCloud<pcl::PointXYZ>::Ptr world_hight_map(new pcl::PointCloud<pcl::PointXYZ>());
